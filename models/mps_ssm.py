...
from core.utils import gaussian_crps, gaussian_kl_divergence, gaussian_nll
...
class MPSSSM(nn.Module):
    ...
    def forward(self, x: Tensor) -> Dict[str, Tensor]:
        ...
        state_prev = torch.zeros(batch, self.d_state, device=device)
    ...
    # Loss and metrics
    # ------------------------------------------------------------------
    def compute_loss(self, outputs: Dict[str, Tensor], target: Tensor, lambda_val: float) -> Dict[str, Tensor]:
        pred_mean = outputs["pred_mean"]
        pred_logvar = outputs["pred_logvar"]

        if self.scoring == "nll":
            pred_loss = gaussian_nll(target, pred_mean, pred_logvar)
        else:
            pred_loss = gaussian_crps(target, pred_mean, pred_logvar)

        mse_loss = F.mse_loss(pred_mean, target)
        mae_loss = F.l1_loss(pred_mean, target)
